...
async function signInWithProvider(provider) {
  // Determine redirect URL (dashboard.html in the same directory)
...
async function getActiveMiningSession(userId) {
  try {
    console.log("getActiveMiningSession: userId=", userId);
    const { data, error } = await supabaseClient
      .from("mining_sessions")
    ...
    console.log("getActiveMiningSession: data=", data, "error=", error);
    if (error) throw error;
    return data && data.length > 0 ? data[0] : null;
  } catch (error) {
    console.error("getActiveMiningSession: exception=", error);
    throw error;
  }
}
